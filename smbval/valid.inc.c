/* mod_ntlm file: $Id: valid.inc.c,v 1.2 2003/02/21 01:55:14 casz Exp $ */
...
Valid_User(char *USERNAME, char *PASSWORD, char *SERVER, char *BACKUP, char *DOMAIN)
{
    ...
    if (SMB_Logon_Server(con, USERNAME, PASSWORD, 0) < 0) {
        SMB_Discon(con, 0);
...
NTLM_Auth(void *handle, char *USERNAME, char *PASSWORD, int flag)
{
    ...
    if (SMB_Logon_Server(con, USERNAME, PASSWORD, flag) < 0) {
        return (NTV_LOGON_ERROR);
    }
    return NTV_NO_ERROR;
}
